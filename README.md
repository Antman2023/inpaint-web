...
验证后台预热与放大模型同时初始化时，设置 `MODEL_SMOKE_WARMUP=1`（PowerShell：`$env:MODEL_SMOKE_WARMUP = '1'`）再运行 `npm run test:models`。测试同时发起修复预热及队列中的放大会话准备，随后验证真实推理与取消恢复。可与环境修复检查组合使用；恢复默认可设置 `$env:MODEL_SMOKE_WARMUP = '0'`。浏览器控制台的 `runBrowserSmoke` 也接受 `exerciseWarmup: true`。

结果中的 `timingsMs` 分别记录取消检查、修复和放大的耗时，包含各自的输出校验。启用预热检查时，模型下载和初始化单独计入 `warmup`；否则启用取消检查时计入 `cancellation`，后续两项使用已准备好的会话。两项检查均未启用时，加载时间计入对应处理步骤。比较性能时应使用相同选项和缓存条件，不要把总耗时差直接视为推理加速。
...
图片解码后会检查 PNG、JPEG、WebP 文件头，纠正支持格式之间误标的 MIME 类型，确保下载扩展名与编码一致；误标为 JPEG 的透明 PNG 在缩小时仍保留透明度。不需要缩放的图片只更正元数据，不重新编码。

示例图片在读取响应内容前检查 HTTP 状态和图片类型，拒绝部分响应（206 或带 Content-Range）及非支持格式，并取消响应流；空内容也会在解码前报错。下载过程中按实际读取字节限制大小，避免持续读取超大响应。

未压缩响应若通过有效的 `Content-Length` 明确声明超过 10 MB，会在读取响应流前直接报错；压缩响应、缺失或不规范的大小声明继续按实际读取字节限制。过小的声明不能绕过限制，远端确认取消的延迟也不会阻碍重新选择图片。
...
修复模型预热不占用图片推理队列：立即点击放大时，无需等待无关的修复模型下载完成。模型初始化仍受到运行环境修复保护，真正的图片推理继续串行执行。

涂抹修图时若模型仍在下载，处理界面会显示下载进度；服务器未提供可用总大小时显示不确定进度。取消后重新涂抹会复用后台下载并立即显示最新进度，不重复下载；缓存命中时不显示下载进度条。
...
放大输出中若出现 `NaN` 或无穷值等无效像素，会停止后续分块并显示处理失败，避免把异常像素转为黑色或白色后写入历史；正常的有限颜色值仍按原规则裁剪到可显示范围。
...
画笔会读取浏览器合并的指针采样点，保留快速曲线的中间路径，同时仍按动画帧合并绘制。浏览器不支持此接口或没有提供采样点时，继续使用普通移动事件。浏览器回归测试比较合并事件与逐点事件的预览像素；不代表真实触控笔设备的精度或延迟测量。

点击“原图”进入对比模式，可在图片上拖动调整对比比例，也可聚焦对比滑块后使用左右方向键。`Home` 显示完整原图，`End` 显示当前编辑结果；窗口缩放时保留对比比例。再次点击“原图”返回涂抹编辑。

对比期间暂停画笔大小调节并隐藏画笔预览；退出对比后恢复调节，保留之前的画笔大小。
...
遮罩关闭要求按下和松开都位于遮罩上；从弹窗内拖出或从遮罩拖入内容区域不会误关闭。浏览器回归覆盖 Chromium、Firefox 和 Playwright WebKit 的鼠标拖动，以及模拟触摸指针捕获的松开位置检查；不等同于真机触摸验证。
